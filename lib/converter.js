...
function convert(options) {
    ...
    page.onLoadFinished = function (status) {
        ...
        try {
            ...
            var dimensions = getSVGDimensions(page, options);
            if (!dimensions) {
...
function getSVGDimensions(page, options) {
    return page.evaluate(function (_options) {
        /* global document: true */
        var el = document.querySelector("svg");
        ...
        if(_options.viewbox) {
            if(_options.retina) {
                return {width: viewBoxWidth * 2, height: viewBoxHeight * 2};
            }
            return {width: viewBoxWidth, height: viewBoxHeight};
        } else {
            if (width && viewBoxHeight) {
                if(_options.retina) {
                    return { width: width * 2, height: (width * viewBoxHeight / viewBoxWidth) * 2};
                } else {
            ...
            if (height && viewBoxWidth) {
                if(_options.retina) {
                    return { width: (height * viewBoxWidth / viewBoxHeight) * 2, height: height * 2};
                } else {
